...
folders mirroring namespaces. Two carve-outs: the host/entry/shell files pinned in place by the
framework (`Program.cs`, `_Imports.razor`, `BaseService.cs`, `ConfigurationExtensions.cs`, and each
Blazor host's `UI/App.razor`) keep the root namespace and get no marker, and the `Database/Core`
project is already the shared root so it is never doubled to `Core.Core`. The rule that actually decides sync is therefore
